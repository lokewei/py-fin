...
def calculate_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "转股溢价率" in df.columns:
        df["转股溢价率_数值"] = (
            df["转股溢价率"].astype(str).str.rstrip("%").astype(float)
        )
    ...
    # 判断是否停牌（换手率=0 或 成交额=0 或 涨跌幅包含"停牌"）
    df["成交额(万元)"] = pd.to_numeric(df["成交额(万元)"], errors="coerce")
    df["换手率_数值"] = (
        df["换手率"].astype(str).str.rstrip("%").replace("-", "0").astype(float)
    )
    df["是否停牌"] = (
        (df["换手率_数值"] == 0)
        | (df["成交额(万元)"] == 0)
        | (df["涨跌幅"].astype(str).str.contains("停牌", na=False))
    )

    # 判断是否高价负收益（价格>200 且 到期收益<0）
    def parse_yield(val):
        val_str = str(val).strip()
        if val_str in ["-", "增强", "nan"]:
            return 0.0
        try:
    ...
    df["到期收益_数值"] = df["到期税前收益"].apply(parse_yield)
    df["是否高价负收益"] = (df["现价"] > 200) & (df["到期收益_数值"] < 0)
...
def screen_avoid_bonds(df: pd.DataFrame) -> dict:
    ...
    # 第三类：一年内到期的双高小市值转债
    results["临期双高小市值"] = df[
        (df["剩余年限_数值"] < 1.0) & (df["是否双高"]) & (df["市值类型"] == "小市值")
...
def screen_focus_bonds(
    df: pd.DataFrame, duration: float = 0.25, hPrice: float = 105
) -> dict:
    """筛选值得关注的转债"""
...
def score_bonds(df: pd.DataFrame, exclude_bank: bool = True) -> pd.DataFrame:
    """对转债进行综合评分"""
    df = df.copy()
    df["综合得分"] = 0
    ...
    # 银行类转债过滤条件
    if exclude_bank:
        df = df[df["回售触发价"].notna()]

    # 正面因素加分
    # 1. 低溢价加分（溢价率越低越好）
    df.loc[df["转股溢价率_数值"] < 10, "综合得分"] += 3
    df.loc[
        (df["转股溢价率_数值"] >= 10) & (df["转股溢价率_数值"] < 20), "综合得分"
    ] += 2
    df.loc[
        (df["转股溢价率_数值"] >= 20) & (df["转股溢价率_数值"] < 30), "综合得分"
    ] += 1
    ...
    # 2. 临期且负收益扣分
    df.loc[
        (df["是否临期"]) & (df["到期税前收益"].astype(str).str.contains("-", na=False)),
        "综合得分",
    ] -= 2
